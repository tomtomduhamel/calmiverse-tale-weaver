...
import { Link, Copy, Loader2, X, Send, UserCheck, Globe, BookOpen } from "lucide-react";
      ...
      <Dialog open={isOpen} onOpenChange={handleClose}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Partagez avec un utilisateur Calmi, générez un lien public ou envoyez sur Kindle
            </DialogDescription>
          </DialogHeader>

          <Tabs defaultValue="calmi" className="w-full">
            <TabsList className="grid w-full grid-cols-3">
              <TabsTrigger value="calmi" className="flex items-center gap-1.5 text-xs sm:text-sm">
                <UserCheck className="h-4 w-4" />
                <span className="hidden sm:inline">Utilisateur</span> Calmi
              </TabsTrigger>
              <TabsTrigger value="public" className="flex items-center gap-1.5 text-xs sm:text-sm">
                <Globe className="h-4 w-4" />
                <span className="hidden sm:inline">Lien</span> Public
              </TabsTrigger>
              <TabsTrigger value="kindle" className="flex items-center gap-1.5 text-xs sm:text-sm">
                <BookOpen className="h-4 w-4" />
                Kindle
              </TabsTrigger>
            </TabsList>

            {/* Onglet Utilisateur Calmi */}
            ...
            {/* Onglet Kindle */}
            <TabsContent value="kindle" className="space-y-4 pt-4">
              <div className="bg-muted/50 rounded-lg p-4">
                <BookOpen className="h-8 w-8 text-muted-foreground mx-auto mb-2" />
                <p className="text-sm text-center text-muted-foreground">
                  Envoyez cette histoire directement sur votre Kindle pour une lecture confortable.
                </p>
                <p className="text-xs text-center text-muted-foreground mt-2">
                  Assurez-vous d'avoir configuré votre email Kindle dans les paramètres.
                </p>
              </div>
