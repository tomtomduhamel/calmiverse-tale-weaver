...
const MinimalStoryCreator: React.FC = () => {
  ...
  return <div className="w-full max-w-4xl mx-auto p-4 animate-fade-in">
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
        ...
        <div className="space-y-4 mb-6">
          <ChildrenSelector children={children} selectedChildrenIds={selectedChildrenIds} onChildToggle={handleChildToggle} error={error} />
          
          {children.length === 0 && <Button variant="outline" className="mt-4" onClick={() => navigate("/profiles")}>
        ...
        <div className="space-y-4 mb-8">
          <ObjectivesSelector objectives={objectives} selectedObjective={selectedObjective} onObjectiveSelect={setSelectedObjective} error={error} />
        </div>
        
        {/* Bouton de soumission */}
        <SubmitButton isSubmitting={isSubmitting} disabled={children.length === 0} />
      </form>
    </div>;
};
